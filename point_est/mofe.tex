...
\begin{thm}[Basu's Theorem] \label{thm: basu_thrm}
    Let $T$ be a complete sufficient statistic for $\bm{\theta}$. If the
    distribution of some statistic $V \left( \bm{X}_1 , \ldots , \bm{X}_n \right)$ does not depend on $\bm{\theta}$, then $V$ is an ancillary statistic (\Cref{defe: ancillary_statistic}). An ancillary statistic will be distributed independently of $T$ \cite{CasellaGeorge2001SI}*{page 287}.
\end{thm}
